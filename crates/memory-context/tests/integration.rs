...
async fn test_full_lifecycle() {
    ...
    mgr.set_value(&root, "platform", b"ai-os".to_vec()).await.unwrap();

    let child = mgr.create_context(Some(root)).await.unwrap();
    mgr.set_value(&child, "task", b"reasoning".to_vec()).await.unwrap();

    assert_eq!(mgr.get_value(&child, "platform").await.unwrap(), Some(b"ai-os".to_vec()));
    assert_eq!(mgr.get_value(&child, "task").await.unwrap(), Some(b"reasoning".to_vec()));
...
async fn test_context_inheritance() {
    ...
    mgr.set_value(&grandparent, "level", b"0".to_vec()).await.unwrap();

    let parent = mgr.create_context(Some(grandparent)).await.unwrap();
    mgr.set_value(&parent, "level", b"1".to_vec()).await.unwrap();

    let child = mgr.create_context(Some(parent)).await.unwrap();
    mgr.set_value(&child, "level", b"2".to_vec()).await.unwrap();

    assert_eq!(mgr.get_value(&child, "level").await.unwrap(), Some(b"2".to_vec()));
    assert_eq!(mgr.get_value(&parent, "level").await.unwrap(), Some(b"1".to_vec()));
    assert_eq!(mgr.get_value(&grandparent, "level").await.unwrap(), Some(b"0".to_vec()));
}
...
async fn test_provider_integration() {
    let ctx_id = TreeContextManager::new().create_context(None).await.unwrap();
...
async fn test_hierarchical_override() {
    let mgr = make_tree();
    let root = mgr.create_context(None).await.unwrap();
    mgr.set_value(&root, "theme", b"dark".to_vec()).await.unwrap();

    let child = mgr.create_context(Some(root)).await.unwrap();
    mgr.set_value(&child, "theme", b"light".to_vec()).await.unwrap();

    assert_eq!(mgr.get_value(&child, "theme").await.unwrap(), Some(b"light".to_vec()));
    assert_eq!(mgr.get_value(&root, "theme").await.unwrap(), Some(b"dark".to_vec()));
}
