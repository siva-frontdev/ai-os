...
impl LessonStore {
    ...
    pub fn store(&self, lesson: Lesson) -> ReflectionResult<()> {
        let mut lessons = self
            .lessons
            .write()
            .map_err(|e| ReflectionError::Internal(e.to_string()))?;
    ...
    pub fn get(&self, lesson_id: &LessonId) -> ReflectionResult<Lesson> {
        ...
        lessons
            .get(lesson_id)
            .cloned()
            .ok_or_else(|| ReflectionError::NoLessons(format!("lesson {:?} not found", lesson_id)))
    }
    ...
    pub fn increment_applied(&self, lesson_id: &LessonId) -> ReflectionResult<()> {
        ...
        if let Some(lesson) = lessons.get_mut(lesson_id) {
            ...
            Err(ReflectionError::NoLessons(format!(
                "lesson {:?} not found",
                lesson_id
            )))
        }
