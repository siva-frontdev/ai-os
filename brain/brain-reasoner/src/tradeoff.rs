...
impl TradeoffAnalyzer {
    ...
    pub fn analyze(&self, hypotheses: &[Hypothesis], _constraints: &[Constraint]) -> ReasonerResult<TradeoffAnalysis> {
        let options: Vec<TradeoffOption> = hypotheses.iter().map(|h| {
        ...
        let recommended = options.iter()
            .max_by(|a, b| (a.benefit - a.cost).partial_cmp(&(b.benefit - b.cost)).unwrap_or(std::cmp::Ordering::Equal))
            .map(|o| o.id.clone());
