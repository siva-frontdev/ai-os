...
impl DefaultResponseCache {
    ...
    pub fn inner_get(&self, key: &CacheKey) -> CacheResult<Option<ModelResponse>> {
        ...
        let Some(mut entry) = self.entries.get_mut(&digest) else {
            return Ok(None);
        };
        if entry.expired() {
            return Ok(None);
...
impl ResponseCache for DefaultResponseCache {
    ...
    async fn invalidate(&self, model_id: &ModelId) -> ModelResult<u64> {
        ...
        let Some(keys) = self.by_model.remove(&model_key).map(|(_, v)| v) else {
            return Ok(0);
        };
        let mut count = 0u64;
        for k in keys {
...
mod tests {
    ...
    fn put_and_get() {
        ...
        cache
            .inner_put(key.clone(), resp.clone(), Duration::from_secs(60))
            .unwrap();
    ...
    async fn response_cache_put_and_get() {
        ...
        cache
            .put(key.clone(), resp.clone(), Duration::from_secs(60))
            .await
            .unwrap();
    ...
    async fn response_cache_invalidate_returns_count() {
        ...
        cache
            .put(key.clone(), resp, Duration::from_secs(60))
            .await
            .unwrap();
